...
import type { IOU, IOUCreate, PaginatedResponse } from "@/types/api";
...
export const useIOUStore = defineStore("ious", () => {
  ...
  const overdueIOUs = computed(() =>
    ious.value.filter(
      (iou) => iou.status === "pending" && new Date(iou.due_date) < new Date(),
    ),
  );
  ...
  const fetchIOUs = async () => {
    ...
    try {
      const response = await iouApi.getAll();
      ious.value = response.results;
    } catch (err: any) {
  ...
  const fetchIOUById = async (id: string) => {
    isLoading.value = true;
    error.value = null;
  ...
  const updateIOU = async (id: string, data: Partial<IOUCreate>) => {
    isLoading.value = true;
    error.value = null;
